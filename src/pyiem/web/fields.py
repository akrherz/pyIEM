...
def _unrectify_wfo(wfo: str | None):
...
def _validate_tz(val: str | None) -> str | None:
...
STATE_LIST_FIELD_OPTIONAL = Annotated[
    ...
    BeforeValidator(lambda v: v.split(",") if isinstance(v, str) else v),
    AfterValidator(
        lambda v: None if v is None else _clean_strings(v, r"^[A-Z]{2}$")
    ),
    _S8F,
    WithJsonSchema({"type": "string"}, mode="serialization"),
]
...
WFO3_FIELD = Annotated[str, BeforeValidator(_unrectify_wfo), _WF]
WFO3_FIELD_OPTIONAL = Annotated[
    str | None, BeforeValidator(_unrectify_wfo), _WF
]
