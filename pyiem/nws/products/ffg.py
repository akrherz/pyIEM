...
SHEFRE = re.compile((r"\.B (?P<src>[A-Z]{3}) (?P<date>[0-9]{6,8}) Z "
                     r"DH(?P<hh>[0-9]{2})/DC(?P<valid>[0-9]{10,12}) "
                     r"/DUE/PPHCF/PPTCF/PPQCF"))
DATARE = re.compile((r"^(?P<ugc>[A-Z0-9]{6})\s+(?P<hour01>[0-9\.]+)/\s+"
...
class FFGProduct(TextProduct):
    ...
    def do_parsing(self):
        ...
        self.issue = datetime.datetime.strptime(group['date'][-6:],
                                                '%y%m%d').replace(
                                                    tzinfo=pytz.utc)
        self.issue = self.issue.replace(hour=(int(group['hh']) % 24))
        dc = datetime.datetime.strptime(group['valid'][-10:],
                                        '%y%m%d%H%M').replace(
                                                              tzinfo=pytz.utc)
        ...
        for match in DATARE.finditer(self.unixtext[pos1:pos2]):
            ...
            rows.append(dict(ugc=group['ugc'],
    ...
    def sql(self, txn):
        ...
        for _, row in self.data.iterrows():
            txn.execute("""
            INSERT into """ + table + """(ugc, valid, hour01, hour03,
            hour06, hour12, hour24) VALUES (%s, %s, %s, %s, %s,
            %s, %s)
            """, (row['ugc'], self.issue, row['hour01'], row['hour03'],
                  row['hour06'], row['hour12'], row['hour24']))
